<script setup>
// Script identical to DomainQualificationTable, just different context
import { computed } from 'vue';
const props = defineProps({ qualificationsData: { type: Array, required: true, default: () => [] } });
const levels = [
...
const reportWithAttention = computed(() => {
    return props.qualificationsData.map(item => {
        ...
        return { ...item, qualificationsWithAttention: { ...counts, Atencion: attentionCount } };
    });
});
const getCellClass = (levelKey, count) => { /* ... same logic ... */
    if (count > 0) {
        const levelConfig = levels.find(l => l.key === levelKey);
        if (levelConfig && ['Bajo', 'Medio', 'Alto', 'Muy Alto'].includes(levelKey)) { return `${levelConfig.color} ${levelConfig.textColor}`; }
    }
    ...
    <div class="overflow-x-auto shadow-md sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-100">
                 <tr>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dimensión</th> <!-- Changed label -->
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="item in reportWithAttention" :key="item.id">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ item.name }}</td> <!-- Dimension name -->
                ...
                <tr v-if="reportWithAttention.length === 0">
                     <td :colspan="levels.length + 2" class="px-6 py-4 text-center text-gray-500">
                         No hay datos de calificación por dimensión disponibles.
                     </td>
                 </tr>
            </tbody>
        </table>
...
<style scoped> td { transition: background-color 0.3s ease; } </style>
